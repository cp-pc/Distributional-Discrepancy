...
class GPT2D(Gens):
    ...
    def init_oracle_trainng(self):
        ...
        with tf.variable_scope(f"oracle_generator_gpt2", reuse=None, initializer=initializer):
            generator = Generator(num_vocabulary=self.vocab_size, batch_size=self.batch_size,
                                  sequence_length=self.sequence_length, \
                                  generator_name=f"oracle_generator_gpt2",
    ...
    def init_real_trainng(self):
        ...
        with tf.variable_scope(f"generator_gpt2", reuse=None, initializer=initializer):
            generator = Generator(num_vocabulary=self.vocab_size, batch_size=self.batch_size,  sequence_length=self.sequence_length, \
                                  generator_name=f"generator_gpt2", start_token=self.start_token, \
                                  learning_rate=self.gen_lr, temperature=self.temperature, n_embd=self.n_embd, n_head=self.n_head, n_layer=self.n_layer, dropout_keep_prob=self.g_dropout_keep_prob)
            ...
            for epoch in range(self.train_gen_num):
                ...
                if (epoch + 1) % self.ntest == 0:
                    ...
                    if values < best_nll:
                        best_nll = values
                        # save pre_train
                        saver.save(self.sess, os.path.join(self.save_path, f'data{int(self.FLAGS.N*100)}_train_gen_best'))
                        print('gen store')
                self.add_epoch()
    ...
    def train_oracle(self):
        ...
        saver_variables = slim.get_variables_to_restore(include=[f"oracle_generator_gpt2"])
        saver = tf.train.Saver(saver_variables, max_to_keep=20)
